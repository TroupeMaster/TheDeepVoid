...
public class ShadestormItem extends Item {
	public ShadestormItem() {
	...
	public void inventoryTick(ItemStack itemstack, Level world, Entity entity, int slot, boolean selected) {
		...
		ShadestormUsedProcedure.execute(world, entity, itemstack);
	}
	...
	public void releaseUsing(ItemStack itemstack, Level world, LivingEntity entity, int time) {
		if (!world.isClientSide() && entity instanceof ServerPlayer player) {
			...
			if (player.getAbilities().instabuild || stack != ItemStack.EMPTY) {
				BigShadeArrowEntity projectile = BigShadeArrowEntity.shoot(world, entity, world.getRandom(), pullingPower);
	...
	private ItemStack findAmmo(Player player) {
		ItemStack stack = ProjectileWeaponItem.getHeldProjectile(player, e -> e.getItem() == BigShadeArrowEntity.PROJECTILE_ITEM.getItem());
		if (stack == ItemStack.EMPTY) {
			for (int i = 0; i < player.getInventory().items.size(); i++) {
				ItemStack teststack = player.getInventory().items.get(i);
				if (teststack != null && teststack.getItem() == BigShadeArrowEntity.PROJECTILE_ITEM.getItem()) {
					stack = teststack;
					break;
				}
			}
