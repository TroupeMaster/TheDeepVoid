...
public class SoulContainerOnTickUpdateProcedure {
		...
		if (!world.getEntitiesOfClass(Player.class, AABB.ofSize(new Vec3(x, y, z), 46, 46, 46), e -> true).isEmpty()) {
			{
				...
				for (Entity entityiterator : _entfound) {
					if (entityiterator instanceof Player && entityiterator.getY() >= y - 1) {
						if (entityiterator instanceof LivingEntity _entity && !_entity.level().isClientSide())
							_entity.addEffect(new MobEffectInstance(TheDeepVoidModMobEffects.WEAVER_CURSE.get(), 30, 0, false, false));
					}
				}
			}
		}
