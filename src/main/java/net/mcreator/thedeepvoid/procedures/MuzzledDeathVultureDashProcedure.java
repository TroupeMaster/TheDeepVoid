...
public class MuzzledDeathVultureDashProcedure {
	public static void execute(LevelAccessor world, Entity entity, ItemStack itemstack) {
		if (entity == null)
			return;
		if ((entity.getVehicle()) instanceof MuzzledDeathVultureEntity && (entity.getVehicle()).onGround()) {
			if (entity instanceof Player _player)
				_player.getCooldowns().addCooldown(itemstack.getItem(), 45);
			if (world instanceof Level _level) {
				if (!_level.isClientSide()) {
					_level.playSound(null, BlockPos.containing((entity.getVehicle()).getX(), (entity.getVehicle()).getY(), (entity.getVehicle()).getZ()), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("the_deep_void:riding_crop_hit")),
			...
			TheDeepVoidMod.queueServerWork(6, () -> {
				if (world instanceof Level _level) {
					if (!_level.isClientSide()) {
						...
						_level.playLocalSound(((entity.getVehicle()).getX()), ((entity.getVehicle()).getY()), ((entity.getVehicle()).getZ()), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("the_deep_void:death_vulture_ambient")),
