...
public class PerilOffHandRightclickedProcedure {
	public static void execute(LevelAccessor world, Entity entity, ItemStack itemstack) {
		...
		if (itemstack.getOrCreateTag().getDouble("shot") > 0 && (entity instanceof Player _plrCldRem3 ? _plrCldRem3.getCooldowns().getCooldownPercent(itemstack.getItem(), 0f) * 100 : 0) <= 0) {
			...
			if (itemstack.getItem() instanceof PerilOffHandItem)
				itemstack.getOrCreateTag().putString("geckoAnim", "empty");
			if (itemstack.getItem() instanceof PerilOffHandItem)
				itemstack.getOrCreateTag().putString("geckoAnim", "animation.peril_shootLeft");
			{
