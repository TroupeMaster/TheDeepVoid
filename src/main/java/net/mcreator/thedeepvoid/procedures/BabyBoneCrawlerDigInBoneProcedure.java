...
public class BabyBoneCrawlerDigInBoneProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if ((world.getBlockState(BlockPos.containing(x, y - 1, z))).is(BlockTags.create(new ResourceLocation("the_deep_void:natural_bone_block")))
				...
				&& (entity instanceof BabyBoneCrawlerEntity _datEntL3 && _datEntL3.getEntityData().get(BabyBoneCrawlerEntity.DATA_dig)) == false) {
			if (entity instanceof BabyBoneCrawlerEntity _datEntSetL)
				_datEntSetL.getEntityData().set(BabyBoneCrawlerEntity.DATA_dig, true);
			if (entity instanceof BabyBoneCrawlerEntity) {
				((BabyBoneCrawlerEntity) entity).setAnimation("animation.babyCrawler_dig");
			}
			TheDeepVoidMod.queueServerWork(20, () -> {
				...
				&& !world.getEntitiesOfClass(Player.class, AABB.ofSize(new Vec3(x, y, z), 10, 10, 10), e -> true).isEmpty()
				&& (entity instanceof BabyBoneCrawlerEntity _datEntL12 && _datEntL12.getEntityData().get(BabyBoneCrawlerEntity.DATA_dig)) == true) {
			if (entity instanceof BabyBoneCrawlerEntity _datEntSetL)
				_datEntSetL.getEntityData().set(BabyBoneCrawlerEntity.DATA_dig, false);
			if (entity instanceof BabyBoneCrawlerEntity) {
				((BabyBoneCrawlerEntity) entity).setAnimation("animation.babyCrawler_out");
			}
