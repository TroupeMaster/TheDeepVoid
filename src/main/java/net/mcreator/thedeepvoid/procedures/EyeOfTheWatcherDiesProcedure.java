...
public class EyeOfTheWatcherDiesProcedure {
		...
		if (sourceentity instanceof Player && !((sourceentity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem() == TheDeepVoidModItems.THE_BONK.get())) {
			TheDeepVoidMod.queueServerWork(20, () -> {
				...
				if (world instanceof ServerLevel _level)
					_level.sendParticles((SimpleParticleType) (TheDeepVoidModParticleTypes.AMBROSIA_DROPLET.get()), x, y, z, 20, 0.01, 0.1, 0.01, 0.1);
				if (world instanceof ServerLevel _level)
					_level.sendParticles((SimpleParticleType) (TheDeepVoidModParticleTypes.LICKER_TONGUE.get()), x, y, z, 6, 0.01, 0.1, 0.01, 0.1);
				if (world instanceof Level _level) {
					if (!_level.isClientSide()) {
						_level.playSound(null, BlockPos.containing(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("the_deep_void:everhunger_scream")), SoundSource.HOSTILE, 2, (float) 0.6);
					} else {
						_level.playLocalSound(x, y, z, ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("the_deep_void:everhunger_scream")), SoundSource.HOSTILE, 2, (float) 0.6, false);
					}
				}
			});
			TheDeepVoidMod.queueServerWork(140, () -> {
				if (world instanceof ServerLevel _level) {
					Entity entityToSpawn = TheDeepVoidModEntities.HIVE_WATCHER.get().spawn(_level, BlockPos.containing(x, y, z), MobSpawnType.MOB_SUMMONED);
				...
				}
				{
					...
					for (Entity entityiterator : _entfound) {
						if (entityiterator instanceof HiveWatcherEntity) {
