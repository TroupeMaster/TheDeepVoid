			...
			for (Entity entityiterator : _entfound) {
				if (entityiterator instanceof LivingEntity) {
					if (!(entityiterator == entity) && !(entityiterator.getDisplayName().getString()).equals(entity.getPersistentData().getString("deep_void:OwnedBy"))) {
						entityiterator.hurt(new DamageSource(world.registryAccess().registryOrThrow(Registries.DAMAGE_TYPE).getHolderOrThrow(DamageTypes.PLAYER_ATTACK), entity), (float) (double) DeepVoidConfigConfiguration.SOULSEEKERTHROWN.get());
						entity.getPersistentData().putDouble("deep_void:scytheBloodAmount", (entity.getPersistentData().getDouble("deep_void:scytheBloodAmount") + 0.05));
						if (world instanceof ServerLevel _level)
