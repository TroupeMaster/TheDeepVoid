...
public class EyeVineStemUpdateTickProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z) {
		if (Math.random() < 0.25 && world.isEmptyBlock(BlockPos.containing(x, y - 1, z))) {
			if (Math.random() < 0.5) {
				...
				if (Math.random() < 0.16) {
					...
					world.setBlock(BlockPos.containing(x, y - 1, z), TheDeepVoidModBlocks.BROWN_EYE_VINE.get().defaultBlockState(), 3);
					if (world instanceof Level _level) {
						if (!_level.isClientSide()) {
							_level.playSound(null, BlockPos.containing(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("block.chorus_flower.death")), SoundSource.BLOCKS, 1, 1);
						} else {
					...
					world.setBlock(BlockPos.containing(x, y - 1, z), TheDeepVoidModBlocks.PURPLE_EYE_VINE.get().defaultBlockState(), 3);
					if (world instanceof Level _level) {
						if (!_level.isClientSide()) {
							_level.playSound(null, BlockPos.containing(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("block.chorus_flower.death")), SoundSource.BLOCKS, 1, 1);
						} else {
