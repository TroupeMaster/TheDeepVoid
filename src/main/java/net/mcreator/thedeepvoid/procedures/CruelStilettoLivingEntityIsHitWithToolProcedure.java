						...
						Math.cos(Math.toRadians(sourceentity.getYRot())), 0.1);
			{
				...
				for (Entity entityiterator : _entfound) {
					if (entityiterator instanceof LivingEntity && !(entityiterator == sourceentity)) {
						entityiterator.hurt(new DamageSource(world.registryAccess().registryOrThrow(Registries.DAMAGE_TYPE).getHolderOrThrow(DamageTypes.MAGIC), sourceentity), (float) (double) DeepVoidConfigConfiguration.CRUELSTILETTO.get());
						entityiterator.setDeltaMovement(new Vec3(0, 0.4, 0));
						if (sourceentity instanceof LivingEntity _entity)
							_entity.setHealth((float) ((sourceentity instanceof LivingEntity _livEnt ? _livEnt.getHealth() : -1) + 0.15));
