			...
			if (entity.getType().is(TagKey.create(Registries.ENTITY_TYPE, new ResourceLocation("the_deep_void:fleshkin"))) == false) {
				...
				if (world instanceof Level _level) {
					if (!_level.isClientSide()) {
						_level.playSound(null, BlockPos.containing(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("entity.shulker_bullet.hit")), SoundSource.BLOCKS, 1, 1);
					} else {
						_level.playLocalSound(x, y, z, ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("entity.shulker_bullet.hit")), SoundSource.BLOCKS, 1, 1, false);
					}
				}
					...
					_level.sendParticles((SimpleParticleType) (TheDeepVoidModParticleTypes.PUS_SPORE.get()), x, (y + 0.5), z, 25, 1, 1, 1, 0.1);
				{
					...
					for (Entity entityiterator : _entfound) {
						if (entityiterator instanceof LivingEntity) {
							...
							if (entityiterator instanceof LivingEntity _entity && !_entity.level().isClientSide())
								_entity.addEffect(new MobEffectInstance(TheDeepVoidModMobEffects.BLOSSOM.get(), 280, 0));
						}
					}
				}
			}
