		...
		if (Math.random() < 0.1) {
			if (!world.getBlockState(BlockPos.containing(x, y + 4, z)).canOcclude() && !world.getBlockState(BlockPos.containing(x, y + 3, z)).canOcclude() && !world.getBlockState(BlockPos.containing(x, y + 2, z)).canOcclude()
				...
				if (world instanceof ServerLevel _serverworld) {
					...
					if (template != null) {
						template.placeInWorld(_serverworld, BlockPos.containing(x - 5, y, z - 5), BlockPos.containing(x - 5, y, z - 5), new StructurePlaceSettings().setRotation(Rotation.NONE).setMirror(Mirror.NONE).setIgnoreEntities(false),
