			...
			return;
		{
			...
			for (Entity entityiterator : _entfound) {
				if (entityiterator instanceof LivingEntity && !(entityiterator == entity)
						&& ((entityiterator instanceof LivingEntity _livEnt ? _livEnt.getMaxHealth() : -1) >= 200
								? (entityiterator instanceof LivingEntity _livEnt ? _livEnt.getHealth() : -1) <= (entityiterator instanceof LivingEntity _livEnt ? _livEnt.getMaxHealth() : -1) / 14
								: (entityiterator instanceof LivingEntity _livEnt ? _livEnt.getHealth() : -1) < (entityiterator instanceof LivingEntity _livEnt ? _livEnt.getMaxHealth() : -1) / 5)) {
					{
						Entity _ent = entity;
						_ent.teleportTo(
					...
					if (world instanceof Level _level) {
						if (!_level.isClientSide()) {
							...
							_level.playLocalSound((entityiterator.getX()), (entityiterator.getY()), (entityiterator.getZ()), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("the_deep_void:soul_seeker_slash")), SoundSource.PLAYERS, 2,
									(float) 0.7, false);
						}
					}
