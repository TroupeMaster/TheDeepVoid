...
public class WandererOnEntityTickUpdateProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		if (entity == null)
			return;
		if (DeepVoidConfigConfiguration.WANDERERCENTERPATHING.get() == true && !(entity instanceof TamableAnimal _tamEnt ? _tamEnt.isTame() : false)) {
		...
		if (!(entity instanceof TamableAnimal _tamEnt ? _tamEnt.isTame() : false)) {
			{
				...
				for (Entity entityiterator : _entfound) {
					if (entityiterator instanceof WandererEntity && !(entityiterator == entity) && !(entityiterator instanceof TamableAnimal _tamEnt ? _tamEnt.isTame() : false)
							&& (entity instanceof WandererEntity _datEntL11 && _datEntL11.getEntityData().get(WandererEntity.DATA_encounter)) == false) {
						...
						TheDeepVoidMod.queueServerWork(80, () -> {
							...
							if (world instanceof Level _level) {
								if (!_level.isClientSide()) {
									_level.playSound(null, BlockPos.containing(entity.getX(), entity.getY(), entity.getZ()), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("particle.soul_escape")), SoundSource.NEUTRAL, 2, 1);
								} else {
									_level.playLocalSound((entity.getX()), (entity.getY()), (entity.getZ()), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("particle.soul_escape")), SoundSource.NEUTRAL, 2, 1, false);
								}
							}
							...
							if ((entity instanceof WandererEntity _datEntL63 && _datEntL63.getEntityData().get(WandererEntity.DATA_dropMoss)) == false) {
								if (entity instanceof WandererEntity _datEntSetL)
		...
		if (entity.isVehicle()) {
			if ((entity.getFirstPassenger()) instanceof LivingEntity _entity && !_entity.level().isClientSide())
				_entity.addEffect(new MobEffectInstance(TheDeepVoidModMobEffects.VOID_BLESSING.get(), 5, 0, false, false));
		}
	}
}
