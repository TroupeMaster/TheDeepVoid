...
public class HivemindDiesProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		if (entity == null)
			return;
		if ((entity instanceof MisanthropicHivemindEntity _datEntL0 && _datEntL0.getEntityData().get(MisanthropicHivemindEntity.DATA_dying)) == false) {
			if (entity instanceof MisanthropicHivemindEntity _datEntSetL)
				_datEntSetL.getEntityData().set(MisanthropicHivemindEntity.DATA_dying, true);
					...
					for (Entity entityiterator : _entfound) {
						if (entityiterator instanceof Player) {
							...
							if (!(entityiterator instanceof ServerPlayer _plr28 && _plr28.level() instanceof ServerLevel
									&& _plr28.getAdvancements().getOrStartProgress(_plr28.server.getAdvancements().getAdvancement(new ResourceLocation("the_deep_void:in_flesh_and_bones"))).isDone())) {
