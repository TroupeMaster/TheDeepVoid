...
public class ApostleSlash1Procedure {
	public static void execute(LevelAccessor world, Entity entity) {
		...
		if (entity instanceof ApostleBossEntity) {
			((ApostleBossEntity) entity).setAnimation("animation.apostle_slash");
		}
					...
					if (!entityiterator.getType().is(TagKey.create(Registries.ENTITY_TYPE, new ResourceLocation("the_deep_void:boss"))) && entityiterator instanceof LivingEntity) {
						if ((entityiterator instanceof LivingEntity _entUseItem50 ? _entUseItem50.getUseItem() : ItemStack.EMPTY).getItem() instanceof ShieldItem) {
							if (entityiterator instanceof Player _player)
								_player.getCooldowns().addCooldown((entityiterator instanceof LivingEntity _entUseItem52 ? _entUseItem52.getUseItem() : ItemStack.EMPTY).getItem(), 40);
							{
		...
		TheDeepVoidMod.queueServerWork(24, () -> {
			...
			}
			{
				...
				for (Entity entityiterator : _entfound) {
					if (!entityiterator.getType().is(TagKey.create(Registries.ENTITY_TYPE, new ResourceLocation("the_deep_void:boss"))) && entityiterator instanceof LivingEntity) {
						if ((entityiterator instanceof LivingEntity _entUseItem84 ? _entUseItem84.getUseItem() : ItemStack.EMPTY).getItem() instanceof ShieldItem) {
							if (entityiterator instanceof Player _player)
								_player.getCooldowns().addCooldown((entityiterator instanceof LivingEntity _entUseItem86 ? _entUseItem86.getUseItem() : ItemStack.EMPTY).getItem(), 40);
							{
								ItemStack _ist = (entityiterator instanceof LivingEntity _entUseItem88 ? _entUseItem88.getUseItem() : ItemStack.EMPTY);
								if (_ist.hurt(4, RandomSource.create(), null)) {
									_ist.shrink(1);
									_ist.setDamageValue(0);
								}
		...
		TheDeepVoidMod.queueServerWork(26, () -> {
			if (entity instanceof ApostleBossEntity _datEntSetL)
