...
public class SkullSmasherEntity extends Monster implements GeoEntity {
	...
	public SkullSmasherEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(TheDeepVoidModEntities.SKULL_SMASHER.get(), world);
	}

	public SkullSmasherEntity(EntityType<SkullSmasherEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
		setMaxUpStep(0.6f);
	...
	public void baseTick() {
		super.baseTick();
		SkullSmasherTickUpdateProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ(), this);
	...
	private PlayState movementPredicate(AnimationState event) {
		if (this.animationprocedure.equals("empty")) {
			if ((event.isMoving() || !(event.getLimbSwingAmount() > -0.15F && event.getLimbSwingAmount() < 0.15F))
				...
				return event.setAndContinue(RawAnimation.begin().thenLoop("animation.skullSmasher_walk"));
			}
			return event.setAndContinue(RawAnimation.begin().thenLoop("animation.skullSmasher_idle"));
		}
	...
	protected void tickDeath() {
		...
		if (this.deathTime == 20) {
			this.remove(SkullSmasherEntity.RemovalReason.KILLED);
			this.dropExperience();
		}
	}
