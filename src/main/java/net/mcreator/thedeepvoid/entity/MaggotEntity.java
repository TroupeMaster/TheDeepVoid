...
import net.mcreator.thedeepvoid.procedures.MaggotThisEntityKillsAnotherOneProcedure;
import net.mcreator.thedeepvoid.procedures.MaggotRightClickedOnEntityProcedure;
import net.mcreator.thedeepvoid.init.TheDeepVoidModEntities;

public class MaggotEntity extends Monster implements GeoEntity {
	...
	public MaggotEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(TheDeepVoidModEntities.MAGGOT.get(), world);
	}

	public MaggotEntity(EntityType<MaggotEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
		setMaxUpStep(0.6f);
	...
	public InteractionResult mobInteract(Player sourceentity, InteractionHand hand) {
		...
		MaggotRightClickedOnEntityProcedure.execute(world, entity, sourceentity, itemstack);
		return retval;
	}
	...
	private PlayState movementPredicate(AnimationState event) {
		if (this.animationprocedure.equals("empty")) {
			if ((event.isMoving() || !(event.getLimbSwingAmount() > -0.15F && event.getLimbSwingAmount() < 0.15F))
				...
				return event.setAndContinue(RawAnimation.begin().thenLoop("animation.maggot_walk"));
			}
			return event.setAndContinue(RawAnimation.begin().thenLoop("animation.maggot_idle"));
		}
	...
	private PlayState attackingPredicate(AnimationState event) {
		...
		if (this.swinging && event.getController().getAnimationState() == AnimationController.State.STOPPED) {
			...
			return event.setAndContinue(RawAnimation.begin().thenPlay("animation.maggot_attack"));
		}
	...
	protected void tickDeath() {
		...
		if (this.deathTime == 20) {
			this.remove(MaggotEntity.RemovalReason.KILLED);
			this.dropExperience();
		}
	}
