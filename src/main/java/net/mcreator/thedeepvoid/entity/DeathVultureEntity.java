...
public class DeathVultureEntity extends Monster implements GeoEntity {
	...
	public DeathVultureEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(TheDeepVoidModEntities.DEATH_VULTURE.get(), world);
	}

	public DeathVultureEntity(EntityType<DeathVultureEntity> type, Level world) {
		super(type, world);
		xpReward = 8;
		setNoAi(false);
		setMaxUpStep(1.2f);
	...
	protected void registerGoals() {
		...
		this.targetSelector.addGoal(10, new NearestAttackableTargetGoal(this, LightEntity.class, false, true));
	}
	...
	public SoundEvent getAmbientSound() {
		return ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("the_deep_void:death_vulture_ambient"));
	}

	@Override
	public SoundEvent getHurtSound(DamageSource ds) {
		return ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("the_deep_void:death_vulture_hurt"));
	}
	...
	public void baseTick() {
		super.baseTick();
		DeathVultureTickUpdateProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ(), this);
	...
	public static void init() {
		SpawnPlacements.register(TheDeepVoidModEntities.DEATH_VULTURE.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
	...
	private PlayState movementPredicate(AnimationState event) {
		if (this.animationprocedure.equals("empty")) {
			if ((event.isMoving() || !(event.getLimbSwingAmount() > -0.15F && event.getLimbSwingAmount() < 0.15F))
					...
					&& !this.isAggressive()) {
				return event.setAndContinue(RawAnimation.begin().thenLoop("animation.vulture_walk"));
			}
	...
	protected void tickDeath() {
		...
		if (this.deathTime == 20) {
			this.remove(DeathVultureEntity.RemovalReason.KILLED);
			this.dropExperience();
		}
	}
