...
import net.mcreator.thedeepvoid.procedures.DeathMawOnEntityTickUpdateProcedure;
import net.mcreator.thedeepvoid.init.TheDeepVoidModEntities;

public class DeathMawEntity extends Monster implements GeoEntity {
	...
	public DeathMawEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(TheDeepVoidModEntities.DEATH_MAW.get(), world);
	}

	public DeathMawEntity(EntityType<DeathMawEntity> type, Level world) {
		super(type, world);
		xpReward = 12;
		setNoAi(false);
		setMaxUpStep(2f);
	...
	public void baseTick() {
		super.baseTick();
		DeathMawOnEntityTickUpdateProcedure.execute(this.level(), this);
	...
	private PlayState movementPredicate(AnimationState event) {
		if (this.animationprocedure.equals("empty")) {
			if ((event.isMoving() || !(event.getLimbSwingAmount() > -0.15F && event.getLimbSwingAmount() < 0.15F))
					...
					&& !this.isAggressive()) {
				return event.setAndContinue(RawAnimation.begin().thenLoop("animation.deathMaw_walk"));
			}
			if (this.isAggressive() && event.isMoving()) {
				return event.setAndContinue(RawAnimation.begin().thenLoop("animation.deathMaw_aggressive"));
			}
			return event.setAndContinue(RawAnimation.begin().thenLoop("animation.deathMaw_idle"));
		}
	...
	private PlayState attackingPredicate(AnimationState event) {
		...
		if (this.swinging && event.getController().getAnimationState() == AnimationController.State.STOPPED) {
			...
			return event.setAndContinue(RawAnimation.begin().thenPlay("animation.deathMaw_attack"));
		}
	...
	protected void tickDeath() {
		...
		if (this.deathTime == 20) {
			this.remove(DeathMawEntity.RemovalReason.KILLED);
			this.dropExperience();
		}
	}
