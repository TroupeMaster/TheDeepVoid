...
public class EyeOfTheWatcherEntity extends Monster implements GeoEntity {
	...
	public EyeOfTheWatcherEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(TheDeepVoidModEntities.EYE_OF_THE_WATCHER.get(), world);
	}

	public EyeOfTheWatcherEntity(EntityType<EyeOfTheWatcherEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
		setMaxUpStep(0.6f);
	...
	protected void defineSynchedData() {
		...
		this.entityData.define(TEXTURE, "eyeofthewatcher");
	}
	...
	public void die(DamageSource source) {
		super.die(source);
		EyeOfTheWatcherDiesProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ(), this, source.getEntity());
	}
	...
	private PlayState movementPredicate(AnimationState event) {
		if (this.animationprocedure.equals("empty")) {
			if (this.isDeadOrDying()) {
				return event.setAndContinue(RawAnimation.begin().thenPlay("animation.eyeOfTheWatcher_death"));
			}
			return event.setAndContinue(RawAnimation.begin().thenLoop("animation.eyeOfTheWatcher_idle"));
		}
	...
	protected void tickDeath() {
		...
		if (this.deathTime == 20) {
			this.remove(EyeOfTheWatcherEntity.RemovalReason.KILLED);
			this.dropExperience();
		}
	}
