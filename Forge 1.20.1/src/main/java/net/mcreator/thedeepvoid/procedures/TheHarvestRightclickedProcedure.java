				...
				if (entityiterator instanceof LivingEntity && !(entityiterator == sourceentity)) {
					...
					if (entityiterator instanceof LivingEntity _entity && !_entity.level().isClientSide())
						_entity.addEffect(new MobEffectInstance(TheDeepVoidModMobEffects.HEXED.get(),
								...
								(int) ((entityiterator instanceof LivingEntity _livEnt && _livEnt.hasEffect(TheDeepVoidModMobEffects.HEXED.get()) ? _livEnt.getEffect(TheDeepVoidModMobEffects.HEXED.get()).getAmplifier() : 0) + 1)));
				}
			}
		}
		repeat = 0;
