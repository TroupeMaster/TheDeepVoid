...
public class EverhungerEntity extends Monster implements GeoEntity {
	...
	public EverhungerEntity(EntityType<EverhungerEntity> type, Level world) {
		super(type, world);
		xpReward = 80;
		setNoAi(false);
	...
	protected void defineSynchedData(SynchedEntityData.Builder builder) {
		...
		builder.define(TEXTURE, "everhungernew");
	}
	...
	public SoundEvent getAmbientSound() {
		return BuiltInRegistries.SOUND_EVENT.get(new ResourceLocation("the_deep_void:everhunger_ambient"));
	}

	@Override
	public SoundEvent getHurtSound(DamageSource ds) {
		return BuiltInRegistries.SOUND_EVENT.get(new ResourceLocation("the_deep_void:everhunger_hurt"));
	}

	@Override
	public SoundEvent getDeathSound() {
		return BuiltInRegistries.SOUND_EVENT.get(new ResourceLocation("the_deep_void:everhunger_chase"));
	}
	...
	public void baseTick() {
		super.baseTick();
		EverhungerOnEntityTickUpdateProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ(), this);
	...
	public static void init(SpawnPlacementRegisterEvent event) {
		event.register(TheDeepVoidModEntities.EVERHUNGER.get(), SpawnPlacementTypes.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, (entityType, world, reason, pos, random) -> {
			int x = pos.getX();
			int y = pos.getY();
			int z = pos.getZ();
			return EverhungerNaturalEntitySpawningConditionProcedure.execute(world, x, y, z);
	...
	private PlayState movementPredicate(AnimationState event) {
		if (this.animationprocedure.equals("empty")) {
			if ((event.isMoving() || !(event.getLimbSwingAmount() > -0.15F && event.getLimbSwingAmount() < 0.15F))
				...
				return event.setAndContinue(RawAnimation.begin().thenLoop("animation.everhunger_walk"));
			}
			return event.setAndContinue(RawAnimation.begin().thenLoop("animation.everhunger_idle"));
		}
	...
	private PlayState attackingPredicate(AnimationState event) {
		...
		if (this.swinging && event.getController().getAnimationState() == AnimationController.State.STOPPED) {
			...
			return event.setAndContinue(RawAnimation.begin().thenPlay("animation.everhunger_attack"));
		}
	...
	protected void tickDeath() {
		...
		if (this.deathTime == 20) {
			this.remove(EverhungerEntity.RemovalReason.KILLED);
			this.dropExperience();
		}
	}
