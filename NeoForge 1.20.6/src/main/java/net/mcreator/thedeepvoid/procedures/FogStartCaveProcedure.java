	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (DeepVoidConfigConfiguration.DEEPVOIDFOG.get() == true) {
			if ((entity.level().dimension()) == ResourceKey.create(Registries.DIMENSION, new ResourceLocation("the_deep_void:deep_void"))) {
				...
				if (entity.getY() <= 40) {
					if (!(entity instanceof LivingEntity _livEnt49 && _livEnt49.hasEffect(TheDeepVoidModMobEffects.HALLUCINATE)) || entity instanceof LivingEntity _livEnt50 && _livEnt50.hasEffect(MobEffects.NIGHT_VISION)) {
						setDistance(60, 80);
						setShape(FogShape.SPHERE);
					}
				}
