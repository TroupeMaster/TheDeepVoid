		...
		if (entity.getPersistentData().getDouble("deep_void:ring") >= 200) {
			...
			if (world instanceof Level _level) {
				if (!_level.isClientSide()) {
					_level.playSound(null, BlockPos.containing(x, y, z), BuiltInRegistries.SOUND_EVENT.get(new ResourceLocation("block.bell.resonate")), SoundSource.NEUTRAL, 4, (float) 0.8);
				} else {
					_level.playLocalSound(x, y, z, BuiltInRegistries.SOUND_EVENT.get(new ResourceLocation("block.bell.resonate")), SoundSource.NEUTRAL, 4, (float) 0.8, false);
				}
			}
		} else {
