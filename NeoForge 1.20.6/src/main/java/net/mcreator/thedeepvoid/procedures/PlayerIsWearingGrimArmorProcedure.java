...
public class PlayerIsWearingGrimArmorProcedure {
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if ((entity instanceof LivingEntity _entGetArmor ? _entGetArmor.getItemBySlot(EquipmentSlot.HEAD) : ItemStack.EMPTY).getItem() == TheDeepVoidModItems.GRIM_ROTTEN_BONE_HELMET.get()
			...
			if (world.getMaxLocalRawBrightness(BlockPos.containing(x, y, z)) < 3 || entity instanceof LivingEntity _livEnt9 && _livEnt9.hasEffect(TheDeepVoidModMobEffects.ROTTEN_HEART)) {
				if (entity.getData(TheDeepVoidModVariables.PLAYER_VARIABLES).darkness_absorption < 200) {
					{
						TheDeepVoidModVariables.PlayerVariables _vars = entity.getData(TheDeepVoidModVariables.PLAYER_VARIABLES);
						_vars.darkness_absorption = entity.getData(TheDeepVoidModVariables.PLAYER_VARIABLES).darkness_absorption + 1;
						_vars.syncPlayerVariables(entity);
					}
				}
				if (entity.getData(TheDeepVoidModVariables.PLAYER_VARIABLES).darkness_absorption >= 100) {
					if (entity instanceof LivingEntity _entity && !_entity.level().isClientSide())
						_entity.addEffect(new MobEffectInstance(MobEffects.DAMAGE_RESISTANCE, 5, 0));
				} else if (entity.getData(TheDeepVoidModVariables.PLAYER_VARIABLES).darkness_absorption >= 200) {
					if (entity instanceof LivingEntity _entity && !_entity.level().isClientSide())
		...
		if ((entity instanceof LivingEntity _entGetArmor ? _entGetArmor.getItemBySlot(EquipmentSlot.HEAD) : ItemStack.EMPTY).getItem() == TheDeepVoidModItems.CULT_HELMET.get()
			...
			if (entity instanceof LivingEntity _entity && !_entity.level().isClientSide())
				_entity.addEffect(new MobEffectInstance(TheDeepVoidModMobEffects.VOID_BLESSING, 60, 0, false, false));
			if (entity instanceof LivingEntity _entity && !_entity.level().isClientSide())
