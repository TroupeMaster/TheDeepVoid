	...
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, Entity sourceentity) {
		...
		if ((sourceentity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem() == Items.BONE) {
			...
			if (entity.getPersistentData().getDouble("deep_void:crawlerStageProgression") >= 64) {
				if (world instanceof Level _level) {
					if (!_level.isClientSide()) {
						_level.playSound(null, BlockPos.containing(x, y, z), BuiltInRegistries.SOUND_EVENT.get(new ResourceLocation("the_deep_void:insect_cry_1")), SoundSource.HOSTILE, 1, (float) 1.4);
					} else {
						_level.playLocalSound(x, y, z, BuiltInRegistries.SOUND_EVENT.get(new ResourceLocation("the_deep_void:insect_cry_1")), SoundSource.HOSTILE, 1, (float) 1.4, false);
					}
				}
				if (world instanceof ServerLevel _level) {
				...
				if (!(sourceentity instanceof ServerPlayer _plr21 && _plr21.level() instanceof ServerLevel
						&& _plr21.getAdvancements().getOrStartProgress(_plr21.server.getAdvancements().get(new ResourceLocation("the_deep_void:miracle_of_life"))).isDone())) {
					if (sourceentity instanceof ServerPlayer _player) {
						AdvancementHolder _adv = _player.server.getAdvancements().get(new ResourceLocation("the_deep_void:miracle_of_life"));
						if (_adv != null) {
