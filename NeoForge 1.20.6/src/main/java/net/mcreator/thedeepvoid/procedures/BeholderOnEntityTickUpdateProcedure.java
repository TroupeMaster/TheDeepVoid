						...
						if (entity.getPersistentData().getDouble("deep_void_call") <= 0) {
							...
							if (world instanceof Level _level) {
								if (!_level.isClientSide()) {
									...
									}.compareDistOf(x, y, z)).findFirst().orElse(null)).getZ()), BuiltInRegistries.SOUND_EVENT.get(new ResourceLocation("the_deep_void:everhunger_scream")), SoundSource.HOSTILE, 10, (float) 1.1);
								} else {
									...
									}.compareDistOf(x, y, z)).findFirst().orElse(null)).getZ()), BuiltInRegistries.SOUND_EVENT.get(new ResourceLocation("the_deep_void:everhunger_scream")), SoundSource.HOSTILE, 10, (float) 1.1, false);
								}
							}
							if (world instanceof Level _level) {
								if (!_level.isClientSide()) {
									...
									}.compareDistOf(x, y, z)).findFirst().orElse(null)).getZ()), BuiltInRegistries.SOUND_EVENT.get(new ResourceLocation("the_deep_void:everhunger_scream")), SoundSource.HOSTILE, 10, (float) 0.9);
								} else {
									_level.playLocalSound((((Entity) world.getEntitiesOfClass(EverhungerEntity.class, AABB.ofSize(new Vec3(x, y, z), 400, 400, 400), e -> true).stream().sorted(new Object() {
										Comparator<Entity> compareDistOf(double _x, double _y, double _z) {
											return Comparator.comparingDouble(_entcnd -> _entcnd.distanceToSqr(_x, _y, _z));
									...
									}.compareDistOf(x, y, z)).findFirst().orElse(null)).getZ()), BuiltInRegistries.SOUND_EVENT.get(new ResourceLocation("the_deep_void:everhunger_scream")), SoundSource.HOSTILE, 10, (float) 0.9, false);
								}
							}
			...
			if (!((entity instanceof Mob _mobEnt ? (Entity) _mobEnt.getTarget() : null).isShiftKeyDown())
				...
				if (!world.getEntitiesOfClass(Player.class, AABB.ofSize(new Vec3(x, y, z), 15, 15, 15), e -> true).isEmpty()) {
					if ((entity instanceof Mob _mobEnt ? (Entity) _mobEnt.getTarget() : null) instanceof LivingEntity _entity && !_entity.level().isClientSide())
						_entity.addEffect(new MobEffectInstance(TheDeepVoidModMobEffects.FIXATION, 5, 0, false, false));
