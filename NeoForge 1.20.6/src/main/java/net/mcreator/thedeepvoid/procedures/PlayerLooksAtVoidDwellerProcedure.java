...
public class PlayerLooksAtVoidDwellerProcedure {
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (DeepVoidConfigConfiguration.STALKED.get() == false) {
			if (entity.getData(TheDeepVoidModVariables.PLAYER_VARIABLES).StalkerCount > 0 && !((entity.level().dimension()) == ResourceKey.create(Registries.DIMENSION, new ResourceLocation("the_deep_void:deep_void")))) {
				{
					TheDeepVoidModVariables.PlayerVariables _vars = entity.getData(TheDeepVoidModVariables.PLAYER_VARIABLES);
