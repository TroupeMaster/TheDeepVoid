...
public class VoidVisionSpyglassUsedProcedure {
	...
	private static void execute(@Nullable Event event, LevelAccessor world, Entity entity, ItemStack itemstack) {
		if (entity == null)
			return;
		if (itemstack.getItem() == Items.SPYGLASS && EnchantmentHelper.getItemEnchantmentLevel(TheDeepVoidModEnchantments.VOID_VISION.get(), itemstack) != 0) {
			{
				...
				for (Entity entityiterator : _entfound) {
					if (!(entityiterator == entity)) {
						if (entityiterator instanceof LivingEntity _entity && !_entity.level().isClientSide())
							_entity.addEffect(new MobEffectInstance(MobEffects.GLOWING, 600, 0));
