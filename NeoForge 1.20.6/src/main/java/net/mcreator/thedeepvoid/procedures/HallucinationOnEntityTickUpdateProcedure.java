...
public class HallucinationOnEntityTickUpdateProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
						...
						if (entityiterator instanceof Player) {
							if (world instanceof Level _level) {
								if (!_level.isClientSide()) {
									_level.playSound(null, BlockPos.containing(entityiterator.getX(), entityiterator.getY(), entityiterator.getZ()), BuiltInRegistries.SOUND_EVENT.get(new ResourceLocation("the_deep_void:hallucination_attack")),
											SoundSource.NEUTRAL, 2, (float) 0.9);
								} else {
									_level.playLocalSound((entityiterator.getX()), (entityiterator.getY()), (entityiterator.getZ()), BuiltInRegistries.SOUND_EVENT.get(new ResourceLocation("the_deep_void:hallucination_attack")), SoundSource.NEUTRAL,
											2, (float) 0.9, false);
								}
							}
							...
							{
								TheDeepVoidModVariables.PlayerVariables _vars = entityiterator.getData(TheDeepVoidModVariables.PLAYER_VARIABLES);
								_vars.NightmareCount = entityiterator.getData(TheDeepVoidModVariables.PLAYER_VARIABLES).NightmareCount + 500;
		...
		if (entity instanceof LivingEntity _entity && !_entity.level().isClientSide())
			_entity.addEffect(new MobEffectInstance(TheDeepVoidModMobEffects.DEAD_INSIDE, 5, 0, false, false));
		...
		if (entity.getPersistentData().getBoolean("deep_void:agressive") == true) {
			if (!world.getEntitiesOfClass(Player.class, AABB.ofSize(new Vec3(x, y, z), 8, 8, 8), e -> true).isEmpty()) {
				if ((entity instanceof LivingEntity _livEnt17 && _livEnt17.hasEffect(TheDeepVoidModMobEffects.LOOKING)) == true) {
