...
public class RotOnEffectActiveTickProcedure {
	public static void execute(LevelAccessor world, Entity entity) {
		...
		if (entity.getPersistentData().getDouble("rotCount") == 15) {
			...
			if ((entity instanceof LivingEntity _livEnt && _livEnt.hasEffect(TheDeepVoidModMobEffects.ROT) ? _livEnt.getEffect(TheDeepVoidModMobEffects.ROT).getAmplifier() : 0) > 1) {
				entity.hurt(new DamageSource(world.registryAccess().registryOrThrow(Registries.DAMAGE_TYPE).getHolderOrThrow(DamageTypes.MAGIC)),
						(float) (0.5 + (entity instanceof LivingEntity _livEnt && _livEnt.hasEffect(TheDeepVoidModMobEffects.ROT) ? _livEnt.getEffect(TheDeepVoidModMobEffects.ROT).getAmplifier() : 0) * 0.5));
			} else {
